...
Most actively controlled bipedal robots use some form of linear inverted pendulum mode (LIPM) in the walking control strategy. This approach approximates the motion of a bipedal robot to that of a pendulum. 

In addition to shifting the COM higher in the sturcture, parallel linked mechanisms provide the benefit of high stiffness and accuracy \cite{Sellaouti2005}. 

It was found that the energy consumption of parallel linked structures is significantly lower than that of its serially linked counter part \cite{Morisawa2000}. 

However, it is much more difficult to determine the singular configurations of the parallel structure without kinematic analysis \cite{Sellaouti2005}. 

ROBIAN is a bipedal robot which utilizes parallel actuation mechanisms at the hip and ankle joints \cite{Konno2002}. 

% The joint torques in parallel mechanisms are distributed almost equally on all actuators. These can be smaller in size which positively affects weight and power consumption of the robot. At the same time a parallel structure increases the stiffness of the joint. Parallel kinematics can be realized with HD gears or ballscrews. With 2 DoF each, the waist and ankle joints are perfectly suited for parallel mechanisms. Also it is desirable to have a parallel mechanism for the hip joint, however, the required work range of this 3-DoF joint makes the realization of an appropriate mechanism more difficult. As the only joint in the leg, the ankle joint is realized in a parallel configuration. \cite{Lohmeier2006} for LOLA
